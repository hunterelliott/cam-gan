...
maxImLoadPerClass = int(6e4) #For datasets too large to fit in memory load a random subset
...
if saveInterval  > 0:
	...
	snapshotDir = '/home/he19/files/CellBiology/IDAC/Projects/CAMELYON/GAN/Snapshots'
...
with tf.variable_scope("discriminators_shared") as scope, tf.device(gpuString):
	...
	for iClass in range(0, nClasses):
		...
		iLoadIms = np.random.randint(0,nFilesCurr-1,nTrainPerClass[iClass]) #Randomize image order in case a subset is loaded
		...
		for iIm in range(0,nTrainPerClass[iClass]):
			trainIms[iClass][:,:,iIm,:] = preProcIm(misc.imread(currTrainDir + os.path.sep + trainFiles[iClass][iLoadIms[iIm]]))
			if iIm%500 == 0:
	...
	loss_D_X = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(forward_D,tf.ones_like(forward_D))) #Discriminator loss on data
	loss_D_GofZ = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(forward_DofG,tf.zeros_like(forward_DofG))) #Discriminator loss on G output
	loss_D = loss_D_X + loss_D_GofZ
	#loss_G = tf.reduce_mean(tf.log(1-forward_DofG))#Generator minimizes
	loss_G = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(forward_DofG,tf.ones_like(forward_DofG))) #Generator loss
